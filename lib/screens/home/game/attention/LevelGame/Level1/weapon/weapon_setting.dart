...
class WeaponSettingV1 {
  ...
  Future<void> load(gameSetting) async {
    // final images = Images();
    Sprite weaponTower = Sprite(await Flame.images.load('weapon/Tower.png'));
    ...
    for (var weaponParam in weaponParams) {
      await Flame.images.load('weapon/${weaponParam['barrelImg0']}.png');
    }
    ...
    for (var weaponParam in weaponParams) {
      ...
      WeaponSetting w = WeaponSetting.empty()
        ..explosion = SpriteSheet.fromColumnsAndRows(
          image: await Flame.images.load(weaponParam['explosionImage']),
          columns: weaponParam['columns'],
          rows: weaponParam['rows'],
        );
      w.fill(gameSetting, weaponParam, tileSize, weaponTower, Flame.images);
