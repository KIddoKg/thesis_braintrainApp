...
class GamebarView extends GameComponent {
  ...
  set killedEnemy(int n) {
    _killedEnemy = n;
    killedStatus.text = 'Cá: $_killedEnemy'; killResult =  _killedEnemy;
  }

  int get missedEnemy => _missedEnemy;
  set missedEnemy(int n) {
    _missedEnemy = n;
    waveStatus.text = 'Điểm: ${(_lives - _missedEnemy * 20) * 50}';
    missedStatus.text = 'Máu: ${_lives - _missedEnemy * 20} %';
    heathResult = (_lives - _missedEnemy * 20) * 50;
    if (_lives - _missedEnemy * 20 <= 0 || _lives - _missedEnemy * 20 < 460) {
