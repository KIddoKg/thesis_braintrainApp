...
import '../LevelGame/Level8/game/game_main.dart';
import '../LevelGame/Level8/game/game_test.dart';
...
class GameAtte3Level8 extends StatefulWidget {
  const GameAtte3Level8({Key? key}) : super(key: key);

  @override
  State<GameAtte3Level8> createState() => _GameAtte3Level8State();
}

class _GameAtte3Level8State extends State<GameAtte3Level8> {
                 ...
                 Row(
                  ...
                  ElevatedButton(
                    style: ButtonStyle(
                      ...
                      MaterialStateProperty.all(const Color.fromARGB(255, 53, 28, 92)) : MaterialStateProperty.all(const Color.fromARGB(
                          255, 105, 104, 104)) ,
    ...
    return FutureBuilder<SharedPreferences>(
      ...
      builder: (BuildContext context, AsyncSnapshot<SharedPreferences> snapshot) {
        if (snapshot.hasData) {
          ...
          int? heathResult = snapshot.data!.getInt('heath_Result')!;          int checkLevel =  snapshot.data!.getInt('AttePass') ?? 0;
          if(checkLevel < 9 ) {
            textUnclock= "Cấp độ 9 đã được mở khóa";
          }else{
            textUnclock ="Quay lại màn hình chính";
          }
          if (killedEnemy != null) {
            sendData(killedEnemy,heathResult,"true", level: 8);
            return WillPopScope(
                onWillPop: () async {
                  Navigator.pushReplacement( context,
                      MaterialPageRoute(builder: (context) => const AttentionScreen()));
