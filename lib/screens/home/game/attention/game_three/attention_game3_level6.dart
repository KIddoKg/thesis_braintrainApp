...
import '../LevelGame/Level6/game/game_main.dart';
import '../LevelGame/Level6/game/game_test.dart';
...
class GameAtte3Level6 extends StatefulWidget {
  const GameAtte3Level6({Key? key}) : super(key: key);

  @override
  State<GameAtte3Level6> createState() => _GameAtte3Level6State();
}

class _GameAtte3Level6State extends State<GameAtte3Level6> {
    ...
    return FutureBuilder<SharedPreferences>(
      ...
      builder: (BuildContext context, AsyncSnapshot<SharedPreferences> snapshot) {
        if (snapshot.hasData) {
          ...
          if(checkLevel < 7 ) {
            textUnclock= "Cấp độ 7 đã được mở khóa";
          }else{
            textUnclock ="Quay lại màn hình chính";
          }
          if (killedEnemy != null) {
            sendData(killedEnemy,heathResult,"true", level: 6);
            return WillPopScope(
                onWillPop: () async {
                  Navigator.pushReplacement( context,
                      MaterialPageRoute(builder: (context) => const AttentionScreen()));
