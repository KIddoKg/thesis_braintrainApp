...
class GameAtte3Level2 extends StatefulWidget {
  const GameAtte3Level2({Key? key}) : super(key: key);

  @override
  State<GameAtte3Level2> createState() => _GameAtte3Level2State();
}

class _GameAtte3Level2State extends State<GameAtte3Level2> {
    ...
    return FutureBuilder<SharedPreferences>(
      ...
      builder: (BuildContext context, AsyncSnapshot<SharedPreferences> snapshot) {
        if (snapshot.hasData) {
          ...
          if(checkLevel < 3 ) {
            textUnclock= "Cấp độ 3 đã được mở khóa";
          }else{
            textUnclock ="Quay lại màn hình chính";
          }
          if (killedEnemy != null) {
            sendData(killedEnemy,heathResult,"true", level: 2);
            return WillPopScope(
                onWillPop: () async {
                  Navigator.pushReplacement( context,
                      MaterialPageRoute(builder: (context) => const AttentionScreen()));
