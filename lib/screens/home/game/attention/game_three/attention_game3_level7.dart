...
import '../LevelGame/Level7/game/game_main.dart';
import '../LevelGame/Level7/game/game_test.dart';
...
class GameAtte3Level7 extends StatefulWidget {
  const GameAtte3Level7({Key? key}) : super(key: key);

  @override
  State<GameAtte3Level7> createState() => _GameAtte3Level7State();
}

class _GameAtte3Level7State extends State<GameAtte3Level7> {
    ...
    return FutureBuilder<SharedPreferences>(
      ...
      builder: (BuildContext context, AsyncSnapshot<SharedPreferences> snapshot) {
        if (snapshot.hasData) {
          ...
          if(checkLevel < 8) {
            textUnclock= "Cấp độ 8 đã được mở khóa";
          }else{
            textUnclock ="Quay lại màn hình chính";
          }
          if (killedEnemy != null) {
            sendData(killedEnemy,heathResult,"true", level: 7);
            return WillPopScope(
                onWillPop: () async {
                  Navigator.pushReplacement( context,
                      MaterialPageRoute(builder: (context) => const AttentionScreen()));
